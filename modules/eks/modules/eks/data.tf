...
data "template_file" "kubeconfig" {
  ...
  vars {
    ...
    kubeconfig_name                   = "${var.environment}.${var.base_domain}"
    endpoint                          = "${aws_eks_cluster.this.endpoint}"
    region                            = "${data.aws_region.current.name}"
    cluster_auth_base64               = "${aws_eks_cluster.this.certificate_authority.0.data}"
    aws_authenticator_command         = "${var.kubeconfig_aws_authenticator_command}"
    aws_authenticator_command_args    = "${var.aws_authenticator_command_args}"
...
data "template_file" "userdata" {
  ...
  vars {
    ...
    bootstrap_extra_args = "${lookup(var.worker_groups[count.index], "bootstrap_extra_args", lookup(var.workers_group_defaults, "bootstrap_extra_args"))}"
    kubelet_extra_args = "${lookup(var.worker_groups[count.index], "kubelet_extra_args", lookup(var.workers_group_defaults, "kubelet_extra_args"))}"
  }
}
